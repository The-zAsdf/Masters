...
__global__ void ADDSLOPE(struct floet *mat, struct floet *sl, double var) {
    extern __managed__ int numElem;
    extern __managed__ ind **threadIndex;
    int i, j, k, l;
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (id < numElem) {
        ...
        k = threadIndex[id]->k;
        l = threadIndex[id]->l;
        if (k == -1 && l == -1) mat->mat[i][j] += sl->mat[i][j]*var;
        else                    mat->ten[i][j][k][l] += sl->ten[i][j][k][l]*var;
    }
}
...
__global__ void SUMDP(struct floet **kM, struct floet *dst) {
    ...
    if (id < numElem) {
        ...
        if (k == -1 && l == -1 && i >= j) {
            ...
            // if (fabs(dst->mat[i][j]) < 0.00001) dst->mat[i][j] = 0.0;
            ...
            if (i < j && k < l) {
                ...
                // if (fabs(dst->ten[i][j][k][l] + num) < 0.00001) {
                //     dst->ten[i][j][k][l] = 0.0;
                //     dst->ten[j][i][k][l] = 0.0;
                //     dst->ten[i][j][l][k] = 0.0;
                //     dst->ten[j][i][l][k] = 0.0;
