...
void readArgs(int argc, char *argv[], Var *v) {
    if (argc != 9) {
        ...
        fprintf(stderr, "Usage: main <W> <J> <D> <h> <S> <N> <e> <c>\n");
        fprintf(stderr, "Given *argv[]:");
...
void outputHamMathematica(const char *fileName, double** mat, double ****ten, int N) {
    FILE *fp;
    char str[255];
    const char *dir1 = "MathematicaInput/H2_";
    const char *dir2 = "MathematicaInput/H4_";
    const char *ext = ".txt";
