...
// __global__ void DOUBLECOPY(struct floet *src, struct floet *dst1, struct floet *dst2) {
//     extern __managed__ int numElem;
//     extern __managed__ ind **threadIndex;
//     extern __managed__ int N;
//     int i, j, k, l;
...
//
//         if (k == -1 && l == -1) {
//             dst1->mat[i][j] = src->mat[i][j];
//             dst2->mat[i][j] = src->mat[i][j];
//         }
//         else {
//             dst1->ten[i][j][k][l] = src->ten[i][j][k][l];
//             dst2->ten[i][j][k][l] = src->ten[i][j][k][l];
//         }
//     }
// }
...
__global__ void GENERATOR(struct floet *src, struct floet *eta) {
    extern __managed__ int numElemOpt;
    extern __managed__ ind **threadIndexOpt;
    extern __managed__ int N;
    int i, j, k, l;
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (id < numElemOpt) {
        ...
        if (k == -1 && l == -1 && i < j) { // if i == j then eta_{ij} = 0
            ...
            // [H0(4), H(4)]
            eta->ten[i][j][k][l] += (src->ten[k][l][k][l]
...
__global__ void DPSLOPE1(struct floet **kM, struct floet *dst, struct floet *m) {
    extern __managed__ int numElemOpt;
    extern __managed__ ind **threadIndexOpt;
    extern __managed__ double h;
    int i, j, k, l;
    ...
    if (id < numElemOpt) {
        ...
        if (k == -1 && l == -1 && i <= j) {
            dst->mat[i][j] = m->mat[i][j] + r_2_9*h*kM[0]->mat[i][j];
            if (i != j) dst->mat[j][i] = dst->mat[i][j];
        } else if (k != -1 && l != -1) {
            if (i < j && k < l) {
                dst->ten[i][j][k][l] = m->ten[i][j][k][l] +
                                       r_2_9*h*kM[0]->ten[i][j][k][l];
...
__global__ void DPSLOPE2(struct floet **kM, struct floet *dst, struct floet *m) {
    extern __managed__ int numElemOpt;
    extern __managed__ ind **threadIndexOpt;
    extern __managed__ double h;
    int i, j, k, l;
    ...
    if (id < numElemOpt) {
        ...
        if (k == -1 && l == -1 && i <= j) {
            dst->mat[i][j] = m->mat[i][j] + r_12*h*(kM[0]->mat[i][j]
                                            + 3.0*kM[1]->mat[i][j]);
            if (i != j) dst->mat[j][i] = dst->mat[i][j];
        } else if (k != -1 && l != -1) {
            if (i < j && k < l) {
                dst->ten[i][j][k][l] = m->ten[i][j][k][l] + r_12*h*(kM[0]->ten[i][j][k][l]
...
__global__ void DPSLOPE3(struct floet **kM, struct floet *dst, struct floet *m) {
    extern __managed__ int numElemOpt;
    extern __managed__ ind **threadIndexOpt;
    extern __managed__ double h;
    int i, j, k, l;
    ...
    if (id < numElemOpt) {
        ...
        if (k == -1 && l == -1 && i <= j) {
            dst->mat[i][j] = m->mat[i][j] + r_324*h*(55.0*kM[0]->mat[i][j]
                            - 75.0*kM[1]->mat[i][j]
                            + 200.0*kM[2]->mat[i][j]);
            if (i != j) dst->mat[j][i] = dst->mat[i][j];
        } else if (k != -1 && l != -1) {
            if (i < j && k < l) {
                dst->ten[i][j][k][l] = m->ten[i][j][k][l] + r_324*h*(55.0*kM[0]->ten[i][j][k][l]
...
__global__ void DPSLOPE4(struct floet **kM, struct floet *dst, struct floet *m) {
    extern __managed__ int numElemOpt;
    extern __managed__ ind **threadIndexOpt;
    extern __managed__ double h;
    int i, j, k, l;
    ...
    if (id < numElemOpt) {
        ...
        if (k == -1 && l == -1 && i <= j) {
            dst->mat[i][j] = m->mat[i][j] + r_330*h*(83.0*kM[0]->mat[i][j]
                            - 195.0*kM[1]->mat[i][j]
                            + 305.0*kM[2]->mat[i][j]
                            + 27.0*kM[3]->mat[i][j]);
            if (i != j) dst->mat[j][i] = dst->mat[i][j];
            ...
            if (i < j && k < l) {
                dst->ten[i][j][k][l] = m->ten[i][j][k][l] + r_330*h*(83.0*kM[0]->ten[i][j][k][l]
...
__global__ void DPSLOPE5(struct floet **kM, struct floet *dst, struct floet *m) {
    extern __managed__ int numElemOpt;
    extern __managed__ ind **threadIndexOpt;
    extern __managed__ double h;
    int i, j, k, l;
    ...
    if (id < numElemOpt) {
        ...
        l = threadIndexOpt[id]->l;
        if (k == -1 && l == -1 && i <= j) {
            dst->mat[i][j] = m->mat[i][j] + r_28*h*(-19.0*kM[0]->mat[i][j]
                            + 63.0*kM[1]->mat[i][j]
                            + 4.0*kM[2]->mat[i][j]
                            - 108.0*kM[3]->mat[i][j]
                            + 88.0*kM[4]->mat[i][j]);
            ...
            if (i < j && k < l) {
                dst->ten[i][j][k][l] = m->ten[i][j][k][l] + r_28*h*(-19.0*kM[0]->ten[i][j][k][l]
...
__global__ void DPSLOPE6(struct floet **kM, struct floet *dst, struct floet *m) {
    extern __managed__ int numElemOpt;
    extern __managed__ ind **threadIndexOpt;
    extern __managed__ double h;
    int i, j, k, l;
    ...
    if (id < numElemOpt) {
        ...
        if (k == -1 && l == -1 && i <= j) {
            dst->mat[i][j] = m->mat[i][j] + 0.0025*h*(38.0*kM[0]->mat[i][j]
                            + 240.0*kM[2]->mat[i][j]
                            - 243.0*kM[3]->mat[i][j]
                            + 330.0*kM[4]->mat[i][j]
                            + 35.0*kM[5]->mat[i][j]);
            ...
            if (i < j && k < l) {
                dst->ten[i][j][k][l] = m->ten[i][j][k][l] + 0.0025*h*(38.0*kM[0]->ten[i][j][k][l]
