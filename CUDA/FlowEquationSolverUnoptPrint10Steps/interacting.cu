...
__global__ void generateMaster(curandState_t* states) {
    ...
    if (id < numElem) {
        ...
        //     master->ten[i][i][k][k] = invGausD[abs(i-k)][r]/2.0;
        //     master->ten[k][k][i][i] = invGausD[abs(i-k)][r]/2.0;
        //     master->ten[i][k][k][i] = -invGausD[abs(i-k)][r]/2.0;
        //     master->ten[k][i][i][k] = -invGausD[abs(i-k)][r]/2.0;
        // } else if (k == -1 && (i == j)) { // init h_i
        //     r = (int)(curand_uniform(&states[id])*1001 - 1);
        //     master->mat[i][j] = (uniform[r]-0.5f)*2.0*W;
...
void DP () {
    ...
    TEST<<<nob, tpb>>>(gen);
    checkCudaSyncErr();
    printf("Testing new condition:\n");
    c = TESTCONDITION(gen);
    printf("Test return: %d", c);
    exit(0);
    ...
    #ifdef DPDEBUG
    enforceAHerm(gen);
    if (!ISANTIHERM(gen, N)) { printf("WARNING: gen not anti-hermitian\n"); }
    #endif
    checkCudaSyncErr();
    #ifdef OUTPUTMAT
    if (count == 0) {
        outputH2("eta2",gen->mat,N);
        outputH4("eta4",gen->ten,N);
    }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(kMat[0]);
    // if (!ISHERM(kMat[0], N)) { printf("WARNING: kMat[0] not hermitian\n"); }
    #endif
    #ifdef OUTPUTMAT
    if (count == 0) {
        outputH2("dH2",kMat[0]->mat,N);
        outputH4("dH4",kMat[0]->ten,N);
        exit(0);
    }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(temp);
    // if (!ISHERM(temp, N)) { printf("WARNING: temp not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(kMat[1]);
    // if (!ISHERM(kMat[1], N)) { printf("WARNING: kMat[1] not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(kMat[2]);
    // if (!ISHERM(kMat[2], N)) { printf("WARNING: kMat[2] not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(kMat[3]);
    // if (!ISHERM(kMat[3], N)) { printf("WARNING: kMat[3] not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(kMat[4]);
    // if (!ISHERM(kMat[4], N)) { printf("WARNING: kMat[4] not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(kMat[5]);
    // if (!ISHERM(kMat[5], N)) { printf("WARNING: kMat[5] not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(kMat[6]);
    // if (!ISHERM(kMat[6], N)) { printf("WARNING: kMat[6] not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(master);
    // if (!ISHERM(master, N)) { printf("WARNING: master not hermitian\n"); }
    #endif
    ...
    #ifdef DPDEBUG
    enforceHerm(temp);
    // if (!ISHERM(temp, N)) { printf("WARNING: error mat not hermitian\n"); }
    #endif
}
...
void embeddedDP () {
    ...
    while (s < l) {
        ...
        for (i = 0; i < ATTEMPTS; i++) {
            ...
            if (round(t) == t && round(t) == 0.0) {
                qq = tol;
            } else {
                qq = fabs(t);
            }
            // scale = 0.2 * sqrt( sqrt ( tol * qq /  (double) err ) );
            scale = 0.8 * sqrt( sqrt ( tol * qq /  (double) err ) );
            scale = min( max(scale,MIN_SCALE_FACTOR), MAX_SCALE_FACTOR);
            if ((double) err < (tol * qq)) break;
            h *= (double) scale;
            if (s + (double) h > l) h = (double)l - (double)s;
            else if (s + (double)h + 0.5*(double)h > l) h = 0.5 * h;
            #ifdef DEBUG
                printf("(h = %.7f, scale = %.3f)\n", h, scale);
            #endif
        ...
        if ( i >= ATTEMPTS ) {
            ...
            if (s + (double) h > l) { last_interval = 1; h = (double) l - (double) s; }
...
double calcInvariant() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < N; j++) {
            t += 0.5*pow(master->mat[i][j],2.0) + master->ten[i][j][i][j];
        }
    }
    return t;
}
...
void enforceHerm(floet *q) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if (i > j) q->mat[j][i] = q->mat[i][j];
            for (int k = 0; k < N; k++) {
                for (int l = 0; l < N; l++) {
                    if (!((i > j && k == l) || (i < j && k <= l))) {
                        if ((i >= j && k > l) || (i == j && k < l)) q->ten[l][k][j][i] = q->ten[i][j][k][l];
                    }
                }
            }
        }
